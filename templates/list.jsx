...
export default function(model, view) {
  const { _columns, _orderedList, _items } = model.toJSON();
      ...
      <div className='component__widget list__widget'>
        <div className={classes([
          ...
          {_items.map(({ _isActive, _imageSrc, alt, title, body }, index) =>
            ...
            ])} role='listitem' style={(hasColumns && Adapt.device.screenSize === 'large' && { width: `${100 / _columns}%` }) || null}>
              <div className='list__item-inner'>
                {!_imageSrc ?
                  <div className='list__item-bullet'></div> :
                  <div className='list__item-image-container'>
                    <img className='list__item-image' src={_imageSrc} aria-label={alt} aria-hidden={!alt} />
                  </div>
                }
